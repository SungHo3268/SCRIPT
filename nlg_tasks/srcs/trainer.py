...
class GPTNLGTrainer(nn.Module):
    ...
    def maybe_logging(self, averager, mode='train'):
        if (mode == 'train') and (len(averager.total) != 0) and (
            ...
            print("")
            if self.logger is not None: self.logger.info(messages)
            self.last_log = time.time()
    ...
    def train(self, **kwargs):
        ...
        for _ in range(self.hparams.optim.epochs):
            ...
            if sum(list(test_score.values())) >= sum(list(self.best_score['best_test_score'].values())):
                print("")
                if self.logger is not None: self.logger.info(f"Save new Best Score (Epoch: {self.current_epoch})")
                ...
                self.stop_cnt = 0
                if self.logger is not None: self.logger.info(f"The Best score is renewed. Stop Count Reset to 0")
                ...
                self.stop_cnt += 1
                print("")
                if self.logger is not None: self.logger.info(f"Stop Count: {self.stop_cnt}")
                if self.stop_cnt == self.hparams.optim.early_stop_patience:
                    self.early_stop = True
                    if self.logger is not None: self.logger.info(f"Early Stop !!!! Training Done.\n")
                    break

        print("\n")
        if self.logger is not None: self.logger.info("########################  BEST RESULT  ########################")
        if self.logger is not None: self.logger.info(f"Epoch: {self.best_score['best_epoch']}")
        if self.logger is not None: self.logger.info("")
        for key in self.best_score['best_dev_score']:
            if self.logger is not None: self.logger.info(f"DEV - {key.upper()}: {self.best_score['best_dev_score'][key] * 100:.2f} [%]")
        if self.logger is not None: self.logger.info("")
        if self.logger is not None: self.logger.info("")
        for key in self.best_score['best_test_score']:
            if self.logger is not None: self.logger.info(f"TEST - {key.upper()}: {self.best_score['best_test_score'][key] * 100:.2f} [%]")
        if self.logger is not None: self.logger.info("##############################################################")
