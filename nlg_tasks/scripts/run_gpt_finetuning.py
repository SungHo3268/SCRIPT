...
def get_config_and_nlg_model(args, tokenizer, logger=None):
    if args.model.hf_model:
        if 'skt/kogpt2' in args.model.name:
            ...
            # model = GPT2LMHeadModel.from_pretrained(args.model.name, config=config, trust_remote_code=True, use_safetensors=True)

            from transformers import AutoModelForSequenceClassification, AutoModelForCausalLM

            if args.data.task_name in ["KB_COPA", "KB_HellaSwag"]:
                base = AutoModelForCausalLM.from_pretrained(
                    args.model.name, config=config, trust_remote_code=True
                )
                base.resize_token_embeddings(len(tokenizer))
                return config, base, tokenizer
            else:
                if args.data.task_name == "KorSTS":
                    config.num_labels = 1
                    ...
                    model_cls = AutoModelForSequenceClassification
                elif args.data.task_name == "KorNLI":
                    config.num_labels = 3
                    config.problem_type = "single_label_classification"
                    model_cls = AutoModelForSequenceClassification
                else:
            ...
            model = model_cls.from_pretrained(args.model.name, config=config, trust_remote_code=True, use_safetensors=True)
            ...
            config = AutoConfig.from_pretrained(args.model.name)
            model = AutoModelWithLMHead.from_pretrained(args.model.name, config=config, trust_remote_code=True, use_safetensors=True)
    ...
    if args.model.set_lora and not args.model.ckpt_dir:
        ...
        is_causal_mc = True if args.data.task_name in ["KB_COPA", "KB_HellaSwag"] else False
    ...
    if args.model.set_script:
        if 'skt/kogpt2' in args.model.name:
            target_modules = ['c_attn', 'c_proj']
            args.model.script.hidden_dim = 768
        elif 'skt/ko-gpt-trinity' in args.model.name:
            target_modules = ['c_attn', 'c_proj']
            args.model.script.hidden_dim = 1920
        elif 'ai-forever/mGPT' in args.model.name:
            target_modules = ['c_attn', 'c_proj']
            args.model.script.hidden_dim = 2048
        elif 'EleutherAI/polyglot-ko' in args.model.name:
            target_modules = ['query_key_value', 'dense']
            args.model.script.hidden_dim = 2048
        elif 'kakaobrain/kogpt' in args.model.name:
            target_modules = ['k_proj', 'v_proj', 'q_proj', 'out_proj']
            args.model.script.hidden_dim = 4096
        else:
            raise NotImplementedError

        # if ('trans' in args.model.script.combination.combination_type) or (args.model.script.do_combination is False and args.model.script.reducer == 'attention_pool'):
        if args.model.set_script:
            trans_config = model.config
            trans_config.update({"is_cross_attention": False})
            trans_config.update({"embedding_norm": args.model.script.embedding_norm})
        else:
            trans_config = None

        if args.model.script.hidden_dim != trans_config.hidden_size:
            args.model.script.hidden_dim = trans_config.hidden_size

        # lora_config = LoRA_Config(
        #     r=args.model.script.lora.r,
        #     lora_alpha=args.model.script.lora.alpha,
        #     lora_dropout=args.model.script.lora.dropout,
        #     target_modules=target_modules
        # )
        script_config = script_Config(
            tok_type=args.model.script.tok_type,
            reducer=args.model.script.reducer,
            hidden_dim=args.model.script.hidden_dim,
            script_max_length=args.model.script.max_length,
            max_length=args.data.max_length,
            do_combination=args.model.script.do_combination,
            combination_type=args.model.script.combination.combination_type,
            trans_config=trans_config,
            num_attention_heads=args.model.script.combination.num_attention_heads,
            intermediate_size=args.model.script.combination.intermediate_size,
            num_trans_layers=args.model.script.combination.num_trans_layers,
            is_bert=False,
            fusion=args.model.script.fusion,
            # lora_config=lora_config
        )
        if args.model.script.tok_type == "same":
            script_tokenizer = tokenizer
            args.model.script.max_length = args.data.max_length
        else:
            script_tokenizer = get_gpt2_tokenizer(
                tok_type=args.model.script.tok_type,
                lang=args.model.script.lang,
                max_length=args.model.script.max_length,
                lowercase=True,
                clean_text=True,
                add_bos_token=False,
                # padding_side='left',
            ...
            if (hasattr(script_tokenizer, "trunc_num") and
                    args.model.script.tok_type in ["jamo_var", "stroke_var", "cji_var", "bts_var"] and
                    args.model.script.max_length % script_tokenizer.trunc_num != 0):
                args.model.script.max_length = args.model.script.max_length - (args.model.script.max_length % script_tokenizer.trunc_num)
                script_tokenizer.max_length = args.model.script.max_length

                if logger is not None:
                    logger.info(f"Change the max_length to {args.model.script.max_length} for the script_tokenizer's truncation.")

        model = apply_script_to_model(model, tokenizer, script_tokenizer, script_config, logger)
        ...
        if args.model.ckpt_dir:
            if args.model.set_lora:
                print("Loading lora checkpoint...", end=' ')
                PeftModel.from_pretrained(model, args.model.ckpt_dir)
                print("Done.!")
            else:
                print("\n")
                if logger is not None:
                ...
                import torch
                state_dict = torch.load(os.path.join(args.model.ckpt_dir, "torch.save.pytorch_model.bin"))
                # print(model.transformer.wte.script_combination.script_embedding.weight[10][:20])
...
def main(args):
    ...
    if args.model.set_script:
        ...
        logger.info(f"ㄴ tok_type             : {args.model.script.tok_type}")
        logger.info(f"ㄴ hidden_dim           : {args.model.script.hidden_dim}")
        logger.info(f"ㄴ script_max_length     : {args.model.script.max_length}")
        logger.info(f"ㄴ embedding_norm       : {args.model.script.embedding_norm}")
        logger.info(f"ㄴ do_combination       : {args.model.script.do_combination}")
        if args.model.script.do_combination:
            logger.info(f"  ㄴ num_attn_heads     : {args.model.script.combination.num_attention_heads}")
            logger.info(f"  ㄴ intermediate_size  : {args.model.script.combination.intermediate_size}")
            logger.info(f"  ㄴ num_trans_layers   : {args.model.script.combination.num_trans_layers}")
        else:
            logger.info(f"ㄴ reducer              : {args.model.script.reducer}\n")
    logger.info('\n')
    if args.model.ckpt_dir:
