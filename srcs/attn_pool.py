...
from transformers.models.gpt2.modeling_gpt2 import (
    GPT2Block, GPT2Attention, eager_attention_forward
)
...
class CustomGPT2Attention(GPT2Attention):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            query = self.q_attn(hidden_states)
            # key = self.k_attn(before_pooled_states)
            # value = self.v_attn(before_pooled_states)
            key, value = self.cv_attn(before_pooled_states).split(self.split_size, dim=2)
...
class CustomBertSelfAttention(BertSelfAttention):
    ...
    def forward(
        ...
        if self.is_decoder:
            # if cross_attention save Tuple(torch.Tensor, torch.Tensor) of all cross attention key/value.
            # Further calls to cross_attention layer can then reuse all cross-attention
            # key/value (first "if" case)
            # if uni-directional self-attention (decoder) save Tuple(torch.Tensor, torch.Tensor) of
            # all previous decoder key/value. Further calls to uni-directional self-attention
            # can concat previous decoder key/value to current projected key/value (third "elif" case)
