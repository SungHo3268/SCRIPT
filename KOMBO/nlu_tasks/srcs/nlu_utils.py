...
from KOMBO.pretraining.srcs.tokenizers import BertTokenizer
from KOMBO.tokenization.srcs.functions import get_tokenizer


def get_bert_script_tokenizer(tok_type, tok_name, max_seq_len):
    custom_tokenizer = get_tokenizer(tok_type, config_path="KOMBO/tokenization/utils/tok_info.cfg")
    if tok_type in ['subword', 'morphemeSubword']:
        custom_tokenizer.load_model(f"KOMBO/tokenization/resources/{tok_name}/tok.model")

    tokenizer = BertTokenizer(vocab_file=f"KOMBO/tokenization/resources/{tok_name}/tok.vocab",
...
def get_bert_tokenizer(args):
    custom_tokenizer = get_tokenizer(args.tok_type, config_path="KOMBO/tokenization/utils/tok_info.cfg")
    if args.tok_type in ['subword', 'morphemeSubword']:
        custom_tokenizer.load_model(f"KOMBO/tokenization/resources/{args.tok_name}/tok.model")

    tokenizer = BertTokenizer(vocab_file=f"KOMBO/tokenization/resources/{args.tok_name}/tok.vocab",
                              custom_tokenizer=custom_tokenizer,
...
def get_config(args):
    if args.model_name in ["kombo-base", "bert-base"]:
        prefix = args.model_name.split("-")[0].lower()

        bert_config_path = f"KOMBO/pretraining/utils/{prefix}_config.json"
    ...
    elif args.model_name == "klue/bert-base":
...
def get_task_model(args, tokenizer):
    ...
    if args.save_dir and not args.load_script:
