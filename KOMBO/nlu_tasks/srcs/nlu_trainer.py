...
from KOMBO.nlu_tasks.srcs.nlu_utils import get_bert_tokenizer, get_task_model, get_optimizer, get_lr_scheduler, get_bert_script_tokenizer, get_config
...
class Trainer(nn.Module):
    def __init__(self, hparams: parser.parse_args, logger):
        ...
        _ = print_trainable_parameters(self.model, logger)
        if hparams.set_script:
            if hparams.script_tok_type in ['subword', 'morphemeSubword']:
                hparams.script_tok_name = f"{hparams.script_tok_type}_{hparams.script_lang}_{hparams.script_bpe_corpus}_{hparams.script_tok_vocab_size}"
            else:
                hparams.script_tok_name = f"{hparams.script_tok_type}_{hparams.script_lang}_{hparams.script_tok_vocab_size}"

            if hparams.script_tok_type == 'same':
                self.script_tokenizer = self.tokenizer
            else:
                self.script_tokenizer = get_bert_script_tokenizer(hparams.script_tok_type, hparams.script_tok_name, hparams.script_max_length)
            target_modules = ['query', 'key', 'value']
            ...
            script_config = script_Config(
                tok_type=hparams.script_tok_type,
                reducer=hparams.script_reducer,
                hidden_dim=hparams.script_hidden_dim,
                script_max_length=hparams.script_max_length,
                max_length=hparams.max_seq_len,
                do_combination=hparams.script_do_combination,
                combination_type=hparams.script_combination_type,
                trans_config=trans_config,
                num_attention_heads=hparams.script_num_attention_heads,
                intermediate_size=hparams.script_intermediate_size,
                num_trans_layers=None,
                is_bert=True,
                fusion=hparams.script_fusion,
                lora_config=lora_config
            )

            if (hasattr(self.script_tokenizer, "trunc_num") and
                    hparams.tok_type in ["jamo_var", "stroke_var", "cji_var", "bts_var"] and
                    hparams.script_max_length % self.script_tokenizer.trunc_num != 0):
                hparams.script_max_length = hparams.script_max_length - (hparams.script_max_length % self.script_tokenizer.trunc_num)
                self.script_tokenizer.max_length = hparams.script_max_length
            logger.info(f"Change the max_length to {hparams.script_max_length} for the script_tokenizer's truncation.")

            self.model = apply_script_to_model(self.model, self.tokenizer, self.script_tokenizer, script_config, logger)
            # make_only_script_and_lora_as_trainable(self.model, weight='script_lora_only', bias='script_lora_only')
            _, _ = print_trainable_parameters(self.model, logger)

        if self.hparams.set_script and self.hparams.load_script and self.hparams.save_dir:
            config = get_config(self.hparams)
            if self.hparams.task_name == "KorQuAD" and hasattr(config, "embedding_type"):
                from KOMBO.pretraining.srcs.models import CustomBertForPreTraining

                model_path = os.path.join(self.hparams.save_dir, "pytorch_model.bin")
                self.model.bert = CustomBertForPreTraining.from_pretrained(model_path, config=config)
                print("Complete to reload the checkpoint of the 'script' model from above save directory.")
            else:
                print(f"Save directory: {self.hparams.save_dir.split('/')[-2]}")
                model_path = os.path.join(self.hparams.save_dir, "pytorch_model.bin")
                ...
                self.model.bert.load_state_dict(bert_state_dict, strict=False)
        ...
        self.model_save_step = self.hparams.total_steps // 20       # save per 5%
        self.model_save_cnt = 0
        self.global_step = 0
        self.cur_ep = 0
        self.stack = 0
    ...
    @staticmethod
    def _shuffle_inputs(inputs: List, labels: List):
        print("Shuffling inputs...", end="")
        data_size = len(inputs)
        data_indices = np.random.permutation(np.arange(data_size))
        shuffled_inputs = [inputs[idx] for idx in data_indices]
        shuffled_labels = [labels[idx] for idx in data_indices]
        print("Done")
    ...
    def fine_tuning(self):
        ...
        if 'kombo' in self.hparams.model_name and self.config.jamo_fusion:
        ...
        if self.hparams.set_script:
            self.logger.info(f"[ script Configuration ]")
            self.logger.info(f"ㄴ script_tok_type         : {self.hparams.script_tok_type}")
            self.logger.info(f"ㄴ script_reducer          : {self.hparams.script_reducer}")
            self.logger.info(f"ㄴ script_hidden_dim       : {self.hparams.script_hidden_dim}")
            self.logger.info(f"ㄴ script_max_length       : {self.hparams.script_max_length}")
            self.logger.info(f"ㄴ script_do_combination   : {self.hparams.script_do_combination}")
            self.logger.info(f"ㄴ script_combination_type : {self.hparams.script_combination_type}")
            self.logger.info(f"ㄴ script_fusion : {self.hparams.script_fusion}\n")

        train_dataset, dev_dataset, test_dataset = self.split_dataset()

        inputs, labels = self.get_input(train_dataset, shuffle=False)
        batch_num = len(inputs)
        ...
        for epoch in range(self.hparams.max_epochs):
            ...
            inputs, labels = self._shuffle_inputs(inputs, labels)
            ...
            for i in tqdm(range(batch_num), desc=f"Fine-tuning...", bar_format=BAR_FORMAT, total=batch_num):
                ...
                if self.global_step % self.model_save_step == 0:
                    # model_save
                    self.model_save_cnt += 1
                    torch.save(self.model.state_dict(), os.path.join(self.hparams.ckpt_dir, f"pytorch_model_{self.model_save_cnt}.bin"))
            ...
            if (dev_acc + test_acc) >= (self.best_ckpt['best_dev_score'] + self.best_ckpt['best_test_score']):
